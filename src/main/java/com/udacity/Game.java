...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        String []ttt = {"X wins", "O wins", "O wins"};
        char [] symbols = {'x', 'o'} ;
        if (grid[0][0] =='x' && grid[1][1] =='x'&& grid[2][2] =='x') {
        	result ="X wins";
    }
        // this tests for diagonals
    if (grid[0][2] == 'x' && grid[1][1] == 'x' && grid[2][0] =='x') {
    		result = "X wins";
    		
    if (grid)[2][0] == 'x' && grid[1][1] == 'x' && grid[2][0] =='x') {
		result = "X wins";
    		
	    
    char [] symbols = {'x', 'o'};
    for (int i=0; i< symbols.lenght; i++) {
    	if (grid[0][0] == symbols [i] == && [1][1] ==symbols[i] && grid [2][2] == symbols [i] )
    	grid[1][1] ;
    }
...
return result; }
    ...
    /**
     * TODO: FOR LOOP ITERATES THROUGH THE BOARD LENGTH
     * inside for loop, check for rows and columns
     * instead of numbers, on of the indices will be a variable
     * 
     * Main function
